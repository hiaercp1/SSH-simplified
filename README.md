...
- Another thing you might want to do is to change the port number from 22 to some other custom port number, this is beacuse it is widely known that ssh services runs on port 22. You can do this by going to the **/etc/ssh/sshd_config** and search for line that says **#Port 22**, uncomment this line and change the number 22 to the number of your desired port. **_Note:_** If you got a firewall you will need to open this port in the firewall.
